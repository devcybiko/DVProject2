...
def get_me_some_age_based_matches():
    ...
    session = Session(engine)
    query = session.query(Matches)
    rows = engine.execute(f"select winner_name as a_winner_name, winner_age as a_winner_age, winner_rank as a_winner_rank, winner_id as a_winner_id, loser_age, loser_id, loser_name from matches where {minage} <= winner_age and winner_age <= {maxage} order by winner_age")
    for row in rows:
        match = dict(row)
        results.append(match)
    return jsonify(results)
